...
OpenTrackerApp.prototype.updatePosition = function() {
	if (this.autocenter) {
		this.map.fitBounds(this.realtime.getBounds(), {
			maxZoom: this.map.DEFAULT_ZOOM
		});
		this.getAddress(this.last_position);
	}
};

OpenTrackerApp.prototype.getAddress = function(position) {
	if (!this.car_is_stopped || !this.autocenter) {
		$.getJSON("https://photon.komoot.de/reverse?lon=" + position.lng + "&lat=" + position.lat,
			function(data) {
				var road = "";
				var city = "";
				if (_.isObject(data) &&
